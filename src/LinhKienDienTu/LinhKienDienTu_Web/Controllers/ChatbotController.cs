...
namespace LinhKienDienTu_Web.Controllers
{
    ...
    public class ChatbotController : ControllerBase
    {
        ...
        private async Task<BotResponse> GetBotResponse(string input)
        {
            ...
            {
                return new BotResponse { Message = "TopGearVN - Gaming Gear Pro Shop hỗ trợ đổi trả trong 3 tháng nếu có lỗi từ nhà sản xuất. 🔄" };
            }
            ...
            if (input.Contains("cho bé") || input.Contains("em bé"))
            {
                ...
                string list = "Dạ, TopGearVN - Gaming Gear Pro Shop có các sản phẩm dinh dưỡng cho bé như:\n" + string.Join("\n", products.Select(p => $"- {p.Ten_San_Pham} ({p.Gia_Goc.ToString("N0")}đ)"));
